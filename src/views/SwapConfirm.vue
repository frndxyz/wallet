    ...
    <div class="wrapper_top form">
      ...
      <div class="form-group">
        ...
        <p class="confirm-value">
          {{ sendTo ? shortenAddress(sendTo) : "Wallet" }}
        </p>
      </div>
      <div class="swap-rate form-group">
        ...
        <p>
          ...
          ><span class="swap-rate_value">&nbsp;{{ rate }}</span
      ...
      <div class="form-group">
        ...
        <div v-for="(fee, asset) in totalFees" :key="asset">
          <strong>
            ...
            <template v-else>Unknown</template> </strong
...
export default {
  ...
  computed: {
    ...
    totalFees() {
      ...
      if (this.toFee) {
        const claimFee = getTxFee(
          this.toAsset,
          TX_TYPES.SWAP_CLAIM,
          this.toFee
        );
        fees[toAssetChain] =
          toAssetChain in fees ? fees[toAssetChain].plus(claimFee) : claimFee;

        if (this.sendTo) {
          const sendFee = getTxFee(this.toAsset, TX_TYPES.SEND, this.toFee);
          fees[toAssetChain] =
            toAssetChain in fees ? fees[toAssetChain].plus(sendFee) : sendFee;
        }
      }
