    ...
    <div class="wrapper_top form">
      ...
      <div v-if="data" class="permission-send_data">
        <label @click="toggleshowData"
          ><ChevronDown
            v-if="showData"
            class="permission-send_data_icon-down"
          /><ChevronRight
...
export default {
  ...
  methods: {
    ...
    async reply(allowed) {
      ...
      if (![2, 3].includes(this.request.args.length))
        throw new Error("Send request must contain 2 or 3 arguments.");
  ...
  computed: {
    ...
    amount() {
      ...
      return cryptoassets[this.asset]
        .unitToCurrency(this.request.args[1])
        .toNumber();
    },
    data() {
