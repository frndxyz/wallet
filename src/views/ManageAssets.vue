  ...
  <div class="manage-assets">
    ...
    <div class="manage-assets_search form wrapper">
      <div class="input-group">
        ...
        />
        <router-link to="/settings/manage-assets/custom-token"
          >Custom Token</router-link
        >
      </div>
    </div>
    <div class="manage-assets_list">
      ...
      >
        ...
        <div class="asset-item_name flex-fill">
          {{ getAssetName(asset) }} ({{ asset }})
          <span v-if="asset in networkWalletBalances" class="asset-item_balance"
            >{{ getAssetBalance(asset) }} {{ asset }}</span
          >
        </div>
    ...
    <div v-if="search" class="wrapper">
      <button
        class="btn btn-light btn-outline-primary btn-lg btn-block"
        @click="clearSearch"
      >
        Done
      </button>
...
export default {
  ...
  computed: {
    ...
    filteredAssets() {
      ...
      return this.assets.filter(
        (asset) =>
          asset.toLowerCase().includes(this.search.toLowerCase()) ||
  ...
  methods: {
    ...
    sortAssets() {
      ...
      this.assets = allAssets.sort(
        (a, b) => this.isAssetEnabled(b) - this.isAssetEnabled(a)
      );
    },
    clearSearch() {
      this.sortAssets();
